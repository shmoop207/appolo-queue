...
class JobsManager extends events_1.EventDispatcher {
    ...
    initialize() {
        this._client.on(events_2.Events.ClientMessage, this._onClientMessage, this);
    }
    start() {
        this._setInterval();
    ...
    _setInterval() {
        ...
        this._interval = (0, timers_1.setInterval)(() => this._checkForJobs(), this._options.checkInterval);
    }
    stop() {
        clearInterval(this._interval);
    ...
    async _checkForJobs() {
        ...
        try {
            ...
            await utils_1.Promises.map(jobsParams, params => this._handleJob(params), { concurrency: maxJobs });
        }
        catch (e) {
            this.fireEvent(events_2.Events.Error, e);
        }
        finally {
        }
    }
    async _handleJob(params) {
        ...
        try {
            ...
            this._client.publish(events_2.Events.JobStart, job.toJobParam());
            ...
            await utils_1.Promises.to(job.nack(e));
        }
        this._jobsRunning--;
        job.destroy();
    }
    ...
    setJobHandler(id, handler, options) {
        options = utils_1.Objects.defaults({}, options, defaults_1.HandlerDefaults);
    ...
    reset() {
        this._client.un(events_2.Events.ClientMessage, this._onClientMessage, this);
    }
}
